            ...
            <div class="card-body">
                <form action="{{ route('sales.update', $sale) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('book_id') is-invalid @enderror" id="book_id" name="book_id" required>
                            ...
                            @foreach($books as $book)
                                <option value="{{ $book->id }}" data-price="{{ $book->prix }}" data-stock="{{ $book->stock }}" 
                                    {{ old('book_id', $sale->book_id) == $book->id ? 'selected' : '' }}>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" min="1" class="form-control @error('quantité') is-invalid @enderror" id="quantité" name="quantité" value="{{ old('quantité', $sale->quantité) }}" required>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control @error('prix_unitaire') is-invalid @enderror" id="prix_unitaire" name="prix_unitaire" value="{{ old('prix_unitaire', $sale->prix_unitaire) }}" required>
                                @error('prix_unitaire')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('date_vente') is-invalid @enderror" id="date_vente" name="date_vente" value="{{ old('date_vente', \Carbon\Carbon::parse($sale->date_vente)->format('Y-m-d')) }}" required>
                        @error('date_vente')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="alert alert-info">
                        <div class="fw-bold">Total: <span id="total-price">{{ number_format($sale->prix_unitaire * $sale->quantité, 2) }}</span> €</div>
                    </div>
